...
class GeminiService extends Service {
    ...
    _safe = true;
    _temperature = 0.9;
    _messages: { role: string; parts: { text: string }[] }[] = [];
    _modelIndex = 0;
    ...
    send(msg) {
        ...
        const proxyResolver = new Gio.SimpleProxyResolver({ default_proxy: null });
        const session = new Soup.Session({ proxy_resolver: proxyResolver });
